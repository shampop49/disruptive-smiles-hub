  ...
  return (
    <Modal title="New Task" onClose={onClose} footer={<><button className="btn bp" style={{flex:1}} onClick={submit}>Create Task</button><button className="btn bg" onClick={onClose}>Cancel</button></>}>
      ...
      <G2>
        ...
        <Field label="Priority"><select className="sel" value={t.priority} onChange={e=>setT(p=>({...p,priority:e.target.value}))}>{PRIORITIES.map(p=><option key={p}>{p}</option>)}</select></Field>
        <Field label="Status"><select className="sel" value={t.status} onChange={e=>setT(p=>({...p,status:e.target.value}))}>{STATUS_OPTIONS.map(s=><option key={s}>{s}</option>)}</select></Field>
        <Field label="Due Date"><input type="date" className="inp" value={t.due} onChange={e=>setT(p=>({...p,due:e.target.value}))}/></Field>
        <Field label="Recurring"><select className="sel" value={t.recur} onChange={e=>setT(p=>({...p,recur:e.target.value}))}>{RECUR_OPTIONS.map(r=><option key={r}>{r}</option>)}</select></Field>
      </G2>
      <Field label="Notes"><textarea className="inp" rows={2} value={t.notes} onChange={e=>setT(p=>({...p,notes:e.target.value}))} style={{resize:"vertical"}}/></Field>
    </Modal>
  );
});
